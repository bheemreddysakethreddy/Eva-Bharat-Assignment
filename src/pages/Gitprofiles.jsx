...
function Gitprofiles() {
    ...
    function handleNavigation(ele){
        localStorage.setItem("searchQuery", query)
        navigate(`/${ele.id}`,{state:{giturl:ele.repos_url, name:ele.login}})
    }
  ...
  return (
    <div className='flex flex-col justify-center items-center w-full'>
        ...
        {!loading && dataItems.length && <div className='w-[80%]'>
            { dataItems?.map((ele)=>(
                <div key={ele.id} onClick={()=>handleNavigation(ele)} className='flex gap-4 mt-2 p-3 shadow rounded-xl hover:shadow-lg shadow-gray-300 cursor-pointer'>
            ...
            <div className='flex gap-10 mb-10 mt-4'>
            <button onClick={()=>setPagination(prev=>({...prev, pagenumber:prev.pagenumber-1}))} className="border rounded" disabled={pagination.pagenumber<=1}>Prev</button>
            <p>{pagination.pagenumber}</p>
            <button onClick={()=>setPagination(prev=>({...prev, pagenumber:prev.pagenumber+1}))} className="border rounded" disabled = {dataItems.length<pagination.pageSize}>Next</button>
        </div>
            </div>}
    </div>
  )
